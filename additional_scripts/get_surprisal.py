...
class Annotations:
    ...
    def load_stimulus_texts(self, stimulus_path) -> pd.DataFrame:
        return pd.read_csv(stimulus_path / "stimuli.tsv", sep="\t", keep_default_na=False,
    ...
    def get_punct_for_wf_files(self):
        ...
        if os.path.exists('surprisal.pickle'):
            ...
            for text_id, sub_df in group_text:
                ...
                word_features_file = pd.read_csv(word_features_file[0], sep="\t", keep_default_na=False,
                ...
                word_features_file.to_csv(
                    f'{self.stimulus_path / "word_features_with_punct"}/word_features_{text_id}.tsv', sep="\t",
                    index=False)
    ...
    def compute_surprisal_from_wf_df(self):
        ...
        if os.path.exists('surprisal.pickle'):
            ...
            all_df = pd.concat([pd.read_csv(file, sep="\t", keep_default_na=False,
                                            na_values=['#N/A', '#N/A N/A', '#NA', '-1.#IND', '-1.#QNAN', '-NaN', '-nan',
                                                       '1.#IND',
                                                       '1.#QNAN', '<NA>', 'N/A', 'NA', 'NaN', 'None', 'n/a', 'nan',
                                                       ''], ) for file in self.word_features_with_punct])
            ...
            for model in multi_scorer.load_models():
                ...
                for id_sn, sub_df in group_text_sent:
                    ...
                    all_df.loc[(all_df['text_id'] == id_sn[0]) & (
                            all_df['sent_index_in_text'] == id_sn[1]), f"sent_surprisal_{model_name}"] = surprisal
            ...
            for file in self.word_features:
                ...
                temp_df = pd.read_csv(file, sep="\t", keep_default_na=False,
                                      na_values=['#N/A', '#N/A N/A', '#NA', '-1.#IND', '-1.#QNAN', '-NaN', '-nan',
                                                 '1.#IND',
                                                 '1.#QNAN', '<NA>', 'N/A', 'NA', 'NaN', 'None', 'n/a', 'nan', ''], )
                ...
                df.to_csv(f'{self.stimulus_path / "word_features_with_surprisal"}/word_features_{text_id}.tsv',
                          sep="\t", index=False)
