# Input: roi-files in aoi/<textid>.ias (nur ein file pro Text, unabhaengig von Leser, weil Fragen irrelevant)
# Fixations (Fixation report created by DataViewer software) in <fixfile> mit mindestens den Spalten [
# 'RECORDING_SESSION_LABEL','itemid','CURRENT_FIX_INDEX', 'CURRENT_FIX_X', 'CURRENT_FIX_Y', 'CURRENT_FIX_DURATION',
# 'CURRENR_FIX_INTEREST_AREA_INDEX'] liste readerIds (hier hart kodiert unten) liste textIds (hier hart kodiert
# unten) in readerIds und Fixation report reader 0_1 umbennen in 0
...
#
# alle fixs werden auf eine roi gemappt (auch diejenigen, die original auf keiner roi sind) oder muessen geloescht
# werden. geloeschte fixs werden nicht gespeichert und die restlichen gereindext (1 .. N)
#
# AusgabeDatei wird beim Fenster schließen erzeugt
#
# Wenn Ausgabedatei vorhanden, wird Trial uebersprungen (also loeschen, falls neu gemacht werde soll oder falls nicht
...
def readFixs(fixfile, readerIds, textIds):
    ...
    for reader in readerIds:
        for text in textIds:
            roifile = '../InterestAreas_Texts/aoi/' + text + '.ias'
...
roifile = ''
reader = ''
fixfile = 'FixRep_20_Mai_2017.txt'
